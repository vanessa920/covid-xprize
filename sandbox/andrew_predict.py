...
MODEL_FILE = os.path.join(ROOT_DIR, "models/xgboost-model")
DATA_FILE = os.path.join(ROOT_DIR, "OxCGRT_latest.csv")
ID_COLS = ['CountryName',
           'RegionName',
           'GeoID',
           'Date']
...
def predict(start_date: str,
    ...
    """
    # !!! YOUR CODE HERE !!!
    preds_df = predict_df(start_date, end_date, path_to_ips_file, verbose=True)
...
def predict_df(start_date_str: str, end_date_str: str, path_to_ips_file: str, verbose=False):
    ...
    for g in ips_df.GeoID.unique():
        ...
        while current_date <= end_date:
            ...
            today_npis = -future_npis[days_ahead:days_ahead + 1]
            X = np.concatenate([X_cases.flatten(),
                                X_npis.flatten(),
                                today_npis.flatten()])

            # Make the prediction (reshape so that sklearn is happy)
            pred = model.predict(xgboost.DMatrix(X.reshape(1, -1)))[0]
            ...
            past_npis = np.append(past_npis, today_npis, axis=0)
